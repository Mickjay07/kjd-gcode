...
export function generateGcode(state) {
  ...
  // BambuStudio-dialect layer markers. Bambu Studio's viewer parses its own
  // dialect ("; CHANGE_LAYER" + "; Z_HEIGHT:" + M73 L) natively and fast;
  // any other producer falls back to slow Z-based layer guessing, which is
  // what made large mesh files lag/freeze the preview.
  function layerMark(n, z, h) {
    push('; CHANGE_LAYER');
    push(`; Z_HEIGHT: ${fmt(z, 3)}`);
    push(`; LAYER_HEIGHT: ${fmt(h, 3)}`);
    push(`M73 L${n + 1}`);
  }
  ...
  // ---------- body ----------
  // The HEADER_BLOCK/CONFIG_BLOCK header is prepended at the end, once the
  // totals (layers, time, filament) are known. The file identifies itself as
  // BambuStudio output so Bambu Studio parses it with its own fast dialect
  // (CHANGE_LAYER / FEATURE tags) instead of the slow generic fallback.
  push('; EXECUTABLE_BLOCK_START');
  push('; FEATURE: Custom');
  ...
  for (let j = 1; j <= printer.bottomLayers; j++) {
    ...
    if (style.bottom === 'lamp') {
      push('; FEATURE: Bottom surface'); // lamp mount ring + spokes
      push(`; LINE_WIDTH: ${lw}`);
  ...
  if (style.wall === 'mesh' && spiralHeight > 0) {
    ...
    if (pts.length >= 3) {
      ...
      for (let i = 3; i < pts.length; i += 3) {
        while (mi < marks.length && marks[mi].idx <= i / 3) {
          ...
          if (mi === 0) { push('; FEATURE: Outer wall'); push(`; LINE_WIDTH: ${printer.nozzle}`); } // open mesh
          mi++;
        }
    ...
    push('; FEATURE: Outer wall'); // spiral vase wall
    push(`; LINE_WIDTH: ${lw}`);
  ...
  // BambuStudio-dialect header, prepended now that the totals are known.
  // Only the format is mirrored (an interoperability requirement — Bambu
  // Studio decides how to parse a file by this header); the values are ours.
  const header = [
    '; HEADER_BLOCK_START',
    '; BambuStudio 02.06.00.51',
    `; generated by KJD GCODE vase designer v${APP_VERSION} (BambuStudio-compatible output)`,
    `; model printing time: ${formatTime(totalTime)}; total estimated time: ${formatTime(totalTime + 360)}`,
    `; total layer number: ${layer}`,
    `; total filament length [mm] : ${filamentMm.toFixed(2)}`,
    `; total filament volume [cm^3] : ${(filamentMm * FILAMENT_AREA / 1000).toFixed(2)}`,
    `; total filament weight [g] : ${filamentG.toFixed(2)}`,
    `; filament_density: ${DENSITY[printer.material] ?? 1.24}`,
    '; filament_diameter: 1.75',
    `; max_z_height: ${fmt(shape.height, 2)}`,
    '; HEADER_BLOCK_END',
    '',
    '; CONFIG_BLOCK_START',
    `; bed_temperature = ${temps.bedTemp}`,
    `; bottom_shell_layers = ${printer.bottomLayers}`,
    `; filament_type = ${printer.material}`,
    '; gcode_flavor = marlin',
    `; initial_layer_print_height = ${lh}`,
    `; layer_height = ${lh}`,
    `; line_width = ${lw}`,
    `; nozzle_diameter = ${printer.nozzle}`,
    `; nozzle_temperature = ${temps.nozzleTemp}`,
    `; printable_area = 0x0,${bedX}x0,${bedX}x${bedY},0x${bedY}`,
    `; printable_height = ${bedZ}`,
    `; printer_model = ${preset.label}`,
